...
public class DriveCommands {
  ...
  private static final double SNAKE_MAX_VELOCITY = 6.0; // Capped rotation speed for snake mode
   ...
   * travel. When the joystick is released, the robot holds its last heading.
   */
  public static Command joystickDriveSnake(
      Drive drive, DoubleSupplier xSupplier, DoubleSupplier ySupplier) {

    // Create PID controller with capped rotation speed
    ProfiledPIDController angleController =
        new ProfiledPIDController(
            ANGLE_KP,
            0.0,
    ...
    // Track last valid heading when joystick was active
    ...
    return Commands.run(
            () -> {
              ...
              if (linearVelocity.getNorm() > 0.01) {
                // Joystick has input - calculate heading from direction of travel
                targetHeading = Math.atan2(linearVelocity.getY(), linearVelocity.getX());
                lastHeading[0] = targetHeading;
              } else {
                // Joystick centered - hold last heading
        ...
        // Reset PID controller and capture current heading when command starts
        .beforeStarting(
            () -> {
              angleController.reset(drive.getRotation().getRadians());
              lastHeading[0] = drive.getRotation().getRadians();
            });
  }
