...
public class IndexerKickerIOTalonFX implements IndexerKickerIO {
  ...
  public IndexerKickerIOTalonFX() {
    talon = new TalonFX(IndexerConstants.kickerMotorId, IndexerConstants.canBus);

    var config = new TalonFXConfiguration();
    config.MotorOutput.NeutralMode = NeutralModeValue.Coast;
    config.MotorOutput.Inverted =
        IndexerConstants.kickerMotorInverted
            ? InvertedValue.Clockwise_Positive
            : InvertedValue.CounterClockwise_Positive;
    config.Slot0 = IndexerConstants.kickerGains;
    config.Feedback.SensorToMechanismRatio = IndexerConstants.kickerGearRatio;
    config.CurrentLimits.StatorCurrentLimit = IndexerConstants.kickerStatorCurrentLimit;
